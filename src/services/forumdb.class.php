...
<?php
  ...
  class ForumDB {
	...
	function save_rating($_forum_id,$_posting_id,$_user_id,$_rating) {
		$this->db->StartTrans();

		// first insert new user rating
      	$sql  = "INSERT {t_user_rating} ";
      	$sql .= " (forum_id, posting_id, user_id, rating)";
        $sql .= " VALUES (";
        $sql .= " {forum_id}, {posting_id}, {user_id}, {rating}";
        $sql .= ")";
	    ...
	    $this->db->_Execute($query->sql()) or die('ForumDB::save_rating(): user_rating');

		// then select all existing ratings to compute new average rating
		$ratings = array();
		$sql  = "SELECT * FROM {t_user_rating}";
		$sql .= " WHERE forum_id={forum_id} AND posting_id={posting_id}";

		$query = new FreechSqlQuery($sql);
	    $query->set_int('forum_id',   $_forum_id);
	    $query->set_int('posting_id', $_posting_id);

		$res = $this->db->_Execute($query->sql()) or die('ForumDB::save_rating(): query');

      	while (!$res->EOF) {
      		$ratings[] = $res->fields["rating"];
      		$res->MoveNext();
      	}

        // now compute average rating
        $average_rating = $this->_compute_average_rating($ratings);

        // save average rating into table
	   $sql  = "UPDATE {t_posting} SET";
     $sql .= " rating={rating}, ";
     $sql .= " rating_count={count} ";
     ...
     $query = new FreechSqlQuery($sql);
	   $query->set_int('forum_id',   $_forum_id);
	   $query->set_int('posting_id', $_posting_id);
	   $query->set_int('rating',     $average_rating);
	   $query->set_int('count',      sizeof($ratings));

     $this->db->_Execute($query->sql()) or die('ForumDB::save_rating(): rating');
     $this->db->CompleteTrans();
	}

	function _compute_average_rating($ratings) {
		if(sizeof($ratings) > 0) {
			$sum = 0;
